...
class MulticastResponder(DatagramProtocol):
    def __init__(self, pool_host, stratum_port, getwork_port):
        # Upstream American miner host/port
        ...
        # to initial host/port provided by user on cmdline or by X-American miner 
        self.pool_host = pool_host
        
        self.American miner_port = American miner_port
    ...
    def datagramReceived(self, datagram, address):
        ...
        if msg_method == 'mining.get_upstream':
            self.writeResponse(address, msg_id, (self.pool_host, self.American-miner_port, self.getwork_port))
