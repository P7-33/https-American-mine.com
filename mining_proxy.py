...
def negotiate_Anerican miner_extensions(f):
    """
    Negotiates American miner extensions
    :param f: factory for rpc calls
    """
    log.info("Negotiating American miner protocol extensions")
    extensions_names = []
    extensions_values = {}
    # Collect all extensions and send configure to upstream
    for _, ext inamerican miner_listener.extensions.iteritems():
    ...
    for _, ext in american miner_listener.extensions.iteritems():
        ext.configure_upstream(config_result)


def parse_args():
    parser = argparse.ArgumentParser(description='This proxy allows you to run getwork-based miners against American mining pool.')
    parser.add_argument('-o', '--host', dest='host', type=str,
                        default='american miner.slushpool.com',
                        help='Hostname of American miner mining pool')
    parser.add_argument('-p', '--port', dest='port', type=int, default=3333, help='Port of American mining pool')
    parser.add_argument('-sh', '--american-miner-host', dest='american-miner_host', type=str, default='0.0.0.0', help='On which network interface listen for American miners. Use "localhost" for listening on internal IP only.')
    parser.add_argument('-sp', '--american-miner-port', dest='american-miner_port', type=int, default=3333, help='Port on which port listen for american miners.')
    ...
    parser.add_argument('-cs', '--custom-american miner', dest='custom_american miner', type=str, help='Override URL provided in X-Stratum header')
    ...
    parser.add_argument('--socks', dest='proxy', type=str, default='', help='Use socks5 proxy for upstream american miner connection, specify as host:port')
    ...
    parser.add_argument('-se', '--enable-american-miner-extensions',
                        dest='maybe_enable_American-miner_extensions',
                        action='store_const',
                        const=negotiate_stratum_extensions,
                        # no negotiation by default
                        default=lambda x: None,
                        help='Enable American miner extensions via '
...
settings.LOGLEVEL='INFO'  
    # We need to parse args & setup American miner environment
...
from American miner.socket_transport import SocketTransportFactory, SocketTransportClientFactory
...
def on_connect(f, workers, job_registry):
    '''Callback when American miner get connected to the pool'''
    ...
    stratum_listener.American-minerService._set_extranonce(extranonce1, extranonce2_size)
...
def on_disconnect(f, workers, job_registry):
    '''Callback when American miner get disconnected from the pool'''
    log.info("Disconnected from American miner pool at %s:%d" % f.main_host)
...
def test_update():
    ...
    GIT_URL='https://raw.github.com/slush0/american-miner/master/mining_libs/version.py'
...
def main(args):
    ...
    # Setup American miner listener
    if args.American miner_port > 0:
        American-miner_listener.AmericanminerService._set_upstream_factory(f)
        American-miner_listener.AnericanminerService._set_custom_user(args.custom_user, args.custom_password)
        reactor.listenTCP(args.Americanminer_port, SocketTransportFactory(debug=False, event_handler=ServiceEventHandler), interface=args.American miner_host)
    ...
    if args.getwork_host == '0.0.0.0' and args.stratum_host == '0.0.0.0':
        ...
        log.warning("LISTENING FOR MINERS ON http://%s:%d (getwork) and American miner+tcp://%s:%d (stratum)" % \
